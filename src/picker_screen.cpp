...
// The picker screen's binding, shared by the cart writer and the games list.
//
// Everything the picker decides is in lib/gbcore/ui/picker.c and everything it
...
//
//   * poll  — one expander read every PICKER_POLL_MS, fed to the picker as a
//             button word and a millis() timestamp
//   * draw  — the display module's canvas, asked for at the per-unit
//             game_x / game_y so the screen renders inside the game window
...
// comments too.
//
// Nothing runs after this screen in the same boot, so the buffers below may be
// generous (see picker_screen.h). The three big ones are on the heap for the
// screen's lifetime rather than static: .bss is reserved in every boot, and
// these 56 KB would otherwise sit idle through every game, in DRAM whose
// largest free block at game time is about 15 KB (TASK-0001). The screen's
// boot runs nothing else, so they come from a heap that has room. The two
// image buffers are separate rather than shared because both images are on
// screen at once, and a partial redraw may repaint either without reading the
// card again.

// One expander read every PICKER_POLL_MS, matching the in-game menu. That is
...
// The title's full description, DESC_MAX on the heap while the screen is up:
...
static enum boot_pick_e screen_run(enum picker_mode_e mode,
    ...
    if (rc != CATALOG_OK && rc != CATALOG_ERR_FULL) {
        ...
        Serial.printf("[PICKER] catalog build failed (%d)\n", rc);
    ...
    if (rc != PICKER_OK) {
        ...
        Serial.printf("[PICKER] layout refused %dx%d (%d)\n", GAME_W, GAME_H,
                      rc);
        return BOOT_PICK_NONE;
    }
    ...
    if (!desc) {
        Serial.printf("[PICKER] no %u B for descriptions\n",
                      (unsigned)DESC_MAX);
    }
    ...
    for (;;) {
        ...
        if (picker_media_due(picker, now, &ci)) {
            ...
            if (!media_logged) {
                ...
                Serial.printf("[PICKER] media %lu us\n",
        ...
        if (ev != PICKER_EVENT_NONE) {
            ...
            if (!logged) {
                ...
                // line per keypress.
                Serial.printf("[PICKER] redraw %lu us\n",
                              (unsigned long)drawn_us);
                logged = true;
            }
        }

        delay(PICKER_POLL_MS);
    }
